...
/**
 * Client event handlers for rendering climber entities.
 * Uses AWCAPI's IAdvancedClimber interface.
 */
public class ClientEventHandlers {
    ...
    private static void renderDebugInfo(LivingEntity entity, IAdvancedClimber climber, Orientation orientation,
        ...
        if (pathingTargets != null && !pathingTargets.isEmpty()) {
            ...
            for (PathingTarget pathingTarget : pathingTargets) {
                ...
                float colorRatio = pathingTargets.size() > 1 ? i / (float) (pathingTargets.size() - 1) : 0;

                LevelRenderer.renderLineBox(matrixStack, bufferIn.getBuffer(RenderType.LINES),
                        new AABB(pos).move(-rx - x, -ry - y, -rz - z),
                        1.0f, colorRatio, 0.0f, 0.15f);
